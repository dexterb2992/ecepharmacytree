...
class VerifyCashPaymentController extends Controller
{
	...
	function verification() {
		...
		foreach($results as $result) {
			...
			if(count($results) == $counter) {
				...
				$order_date = $order_date->toFormattedDateString();
				$order_time = $order_date->format('h:i A');
				$this->emailtestingservice($email, $order_details, $recipient_name, $recipient_address, $recipient_contactNumber, $payment_method, $modeOfDelivery, $coupon_discount, $points_discount, $totalAmount_final, $gross_total, $order_id, $order_details, $order_date, $status, $order_time);				
			}
		}
	...
	function emailtestingservice($email, $order_details, $recipient_name, $recipient_address, $recipient_contactNumber, $payment_method, $modeOfDelivery, $coupon_discount, $points_discount, $totalAmount_final, $gross_total, $order_id, $order_details, $order_date, $status, $order_time){
		$res = $this->mailer->send( 'emails.invoice_remastered', 
			compact('email', 'recipient_name', 'recipient_address', 'recipient_contactNumber', 'payment_method', 'modeOfDelivery', 'coupon_discount', 'points_discount', 'totalAmount_final', 'gross_total', 'order_id', 'order_details', 'order_date', 'status', 'order_time'), function ($m) use ($email) {
